...
from core.DockerProcess import *
from api.server import start_api_server
from config import (docker_configuration, network_configuration, user_configuration)
from core.alert import error, info
from core.color import reset_cmd_color
from core.check import check_for_requirements
...
from database.connector import (push_events_queues_to_database,
                                ...
                                create_indices, insertDockerData)
...
def wait_until_interrupt(virtual_machine_container_reset_factory_time_seconds, configuration,
                         new_network_events_thread, run_as_test, docker):
    """
            ...
            if containers_are_unhealthy(configuration):
                if is_verbose_mode():
                    for container in containers_are_unhealthy(configuration):
                        os.system(f"docker logs {container}")
                return error(
...
def load_honeypot_engine():
    ...
    logo()    # print logo
    parser, argv_options = argv_parser()  # parse argv
    ...
    docker.create_ohp_networks()
    # start containers based on selected modules
    configuration = start_containers(configuration)
    # the data for drawing gauge in api server 
    insertDockerData(docker.count_docker_containers(), docker.count_docker_images())
