...
def get_gateway_ip_addresses(configuration):
    ...
    for selected_module in configuration:
        ...
        try:
            ...
            warn(messages["unable_to_get_ip"].format(container_name))
    ...
    try:
        ...
        if "IP" in packet:
            ...
            if netaddr.valid_ipv4(ip_dest) or netaddr.valid_ipv6(ip_dest):
                ...
                # # honeypot_ports : {8080: 'http/apache_vul'}

                if port_dest in honeypot_ports:  # or port_src in honeypot_ports:
                    # with open("/opt/Python-Honeypot/tmp/test_http.log", 'a+') as fp:
                    #     fp.write(f"{ip_src}:{port_src}\t{ip_dest}:{port_dest}\n")
                    #     fp.write(f"{honeypot_ports}\n")
                    #     fp.write(f"{'HTTP' in packet}\t{hasattr(packet.http, 'request')}\n")
                    #     fp.write(f"{packet.http.request_method}\t") # POST
                    #     fp.write(f"{packet.http.request_uri}\t") # /hello?id=1
                    #     fp.write(f"{packet.http.file_data}\n") # username=admin

                    if 'HTTP' in packet:
                        machine_name = packet.http.request_uri if hasattr(packet.http, 'request_uri') else ""
                        protocol = packet.http.file_data if hasattr(packet.http, 'file_data') else ""
                    ...
                    insert_to_honeypot_events_queue(
                        HoneypotEvent(
                            ...
                            ip_src, port_src,
                            protocol,
                            # honeypot_ports[port_dest if port_dest in honeypot_ports else port_src],
...
def network_traffic_capture(configuration, honeypot_events_queue, network_events_queue, network_config):
    ...
    # with open("/opt/Python-Honeypot/tmp/test.log", 'a+') as fp:
    #     fp.write(f"{honeypot_ports}\n")
    #     fp.write(f"{virtual_machine_ip_addresses}\n")
    ...
    while True:
        ...
        output_file_path = os.path.join(base_dir_path,
                                        "captured-traffic-" + str(file_timestamp) + ".pcap"
                                        )

        if store_pcap:
            info(messages["network_capture_getting_stored"].format(output_file_path))
