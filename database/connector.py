...
def insertDockerData(total_running, total_images):
    print(f"total running honeypot: {total_running},  total honeypot images: {total_images}")
    elasticsearch_events.update(
        index="data_events", id="gauge1", 
        body={
            "doc": {
                "module_name": total_running,
                "machine_name": total_images
            }
        }
    ) 
