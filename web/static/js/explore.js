...
function get_event_data(api_endpoint, column_list, api_params) {
    // var dateColumnIndex = column_list.findIndex(column => column.data === 'date');
...
function load_data(api_endpoint, api_params) {
    ...
    if (api_params.event_type === "honeypot") {
        let title1 = "Severity", title2 = "tcp";
        if (/^http.*$/.test(api_params.module_name)) {
            title1 = "URI";
            title2 = "Request Body";
        } 
        // 原顺序: country_ip_dest, country_ip_src, date, ip_dest, ip_src, module_name, machine_name, port_dest, port_src, protocol
        columns = [
            ...
            {data: 'machine_name', defaultContent: '', title: title1},
            {data: 'protocol', defaultContent: '', title: title2},
